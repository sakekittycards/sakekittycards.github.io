"""
One-shot: push freshly re-cropped front+back images for 4 graded listings
that had bad first-pass crops. Hits POST /admin/replace-graded-images on
the worker which deletes the old image objects and uploads new ones.

Usage:
    SK_ADMIN_TOKEN=<token> python replace_images.py
"""
...
def main():
    ...
    for job in JOBS:
        ...
        if not front.exists() or not back.exists():
            print(f'SKIP {cert}: missing {front.name} or {back.name}')
            continue
