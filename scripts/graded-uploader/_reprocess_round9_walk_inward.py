"""
Round 5: re-run every mapped cert through the alpha-hardened rembg
pipeline. Round 4 produced a feathered slab edge that read as melted
plastic against the dark backdrop — now hardened to crisp 0/255 with
a 1px AA pass.

Reads from _cert_to_imgs.json (built across the previous OCR rounds),
processes every mapped cert in one go.
"""
...
def main():
    ...
    for cert, imgs in sorted(cert_to_imgs.items()):
        ...
        if not out_dir.exists():
            continue
        jobs.append({'cert': cert, 'slug': slug, 'front': imgs[0], 'back': imgs[1], 'out': out_dir})

    print(f'Round 5: {len(jobs)} cards to reprocess')
    t_start = time.time()
